...
var Project = {
	...
	stopScroll: function () {
		$(window).on('scroll', function () {
			$('.blogpost-link').each(function () {
				if (($(window).scrollTop() + $(window).height()) > ($('.blogpost-back-button').offset().top + $(window).height()/2)) {
					$(this).css('top' , $('.blogpost-back-button').offset().top - $('header').height()).addClass('atbottom');
				} else {
					$(this).css('top', '').removeClass('atbottom');
				}
