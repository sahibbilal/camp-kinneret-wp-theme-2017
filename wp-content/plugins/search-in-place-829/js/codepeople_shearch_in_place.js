							...
							results["result"]["source"].push(
								{
									"link"   : '#'+bookmark,
									"title"  : text,
									"resume" : nodeData.substring(Math.max(0,patternIndex-('search_in_place_characters_before' in window ? search_in_place_characters_before : 10)), Math.min(patternIndex+pattern.length+('search_in_place_characters_after' in window ? search_in_place_characters_after : length),nodeData.length))+'<span class="ellipsis">[...]</span>'
								}
							);
							skip = 1;
						}
